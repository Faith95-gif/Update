...
// Setup meeting booking functionality
const bookingAPI = setupMeetingBooking(app, io);

// Setup meeting activity functionality (NEW)
const meetingActivityAPI = setupMeetingActivity(app, io);

// Setup meeting statistics functionality
setupMeetingStatsRoutes(app);
const meetingStatsAPI = setupMeetingStatsSocket(io);

// Setup meeting participant statistics functionality
setupMeetingParticipantStatsRoutes(app);

// Add a test endpoint to manually record participants (for debugging)
app.post('/api/test-record-participant', async (req, res) => {
  try {
    ...
    const { recordMeetingParticipant } = await import('./modules/meetingParticipantStats.js');
    const result = await recordMeetingParticipant(userId, 1);
    
    res.json({ 
      message: 'Test participant recorded successfully', 
      result,
...
// Meeting activity dashboard route (NEW) - For viewing meeting history/analytics
...
io.on('connection', (socket) => {
  ...
  // Setup meeting booking handlers for this socket
  ...
  const { handleDisconnect: handleActivityDisconnect } = meetingActivityAPI.setupSocketHandlers(socket);
  ...
  socket.on('disconnect', () => {
    ...
    // Handle booking cleanup
    handleBookingDisconnect();
    
    // Handle meeting activity cleanup (NEW)
...
server.listen(PORT, () => {
    ...
    console.log(`Meeting Activity: http://localhost:${PORT}/activity`);
    ...
    console.log('Meeting booking functionality integrated');
    console.log('Meeting activity tracking integrated'); // NEW
