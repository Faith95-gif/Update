...
export const recordMeetingParticipant = async (hostUserId, participantCount = 1) => {
  try {
    if (!hostUserId) {
      console.error('No hostUserId provided to recordMeetingParticipant');
      return null;
    }
    ...
    // Find or create stats record for present month
    ...
    console.log(`✅ Recorded ${participantCount} participant(s) for user ${hostUserId} in ${year}-${month}. Total this month: ${stats.totalParticipants}`);
    return stats;
  } catch (error) {
...
// Function to get present and previous month stats for comparison
export const getMeetingParticipantStats = async (userId) => {
  try {
    ...
    // Get present month stats
    const presentStats = await MeetingParticipantStats.findOne({
      userId,
      year: presentYear,
      month: presentMonth
    });
    ...
    const presentParticipants = presentStats?.totalParticipants || 0;
    ...
    if (previousParticipants === 0 && presentParticipants > 0) {
      percentageChange = 100;
      changeDirection = 'up';
    } else if (previousParticipants > 0) {
      percentageChange = Math.round(((presentParticipants - previousParticipants) / previousParticipants) * 100);
    ...
    return {
      present: {
        ...
        meetings: presentStats?.meetingsHosted || 0,
        year: presentYear,
        month: presentMonth
      },
      previous: {
...
export const setupMeetingParticipantStatsRoutes = (app) => {
  // Get present user's meeting participant stats
