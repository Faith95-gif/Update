...
// Updated Meeting Schema with proper date handling
const meetingSchema = new mongoose.Schema({
    title: { type: String, required: true },
    date: { type: String, required: true }, // Keep as string for display
    time: { type: String, required: true }, // Keep as string for display
    dateTime: { type: Date, required: true }, // Add proper DateTime field
...
// Helper function to create proper DateTime from date and time strings
...
const setupSchedulerRoutes = (app) => {
    ...
    app.post('/api/schedule-meeting', async (req, res) => {
        try {
            ...
            const meeting = new Meeting({
                ...
                date,
                time,
                dateTime, // Store proper DateTime
                duration,
            ...
            await meeting.save();
            console.log(`✅ Meeting saved to database: ${meeting.title} at ${dateTime}`);
    ...
    // Get upcoming meetings - FIXED QUERY
    app.get('/api/meetings/upcoming', async (req, res) => {
        try {
            ...
            // Find meetings where user is organizer or participant AND meeting is in the future
            const meetings = await Meeting.find({
                $and: [
                    ...
                    {
                        dateTime: { $gt: now } // Simply check if meeting dateTime is after now
                    }
                ]
            ...
            meetings.forEach(meeting => {
                console.log(`  - ${meeting.title}: ${meeting.dateTime} (${meeting.dateTime > now ? 'FUTURE' : 'PAST'})`);
            });
    ...
    // Add a debug endpoint to check all meetings
    app.get('/api/meetings/debug', async (req, res) => {
        try {
            const now = new Date();
            const allMeetings = await Meeting.find({}).sort({ dateTime: 1 });
...
// Initialize reminder system - FIXED
const initializeReminderSystem = (io) => {
    ...
    cron.schedule('* * * * *', async () => {
        try {
            ...
            // Find meetings that start between 1-5 minutes from now and haven't been notified
            ...
            for (const meeting of meetingsToRemind) {
                console.log(`📧 Sending reminders for meeting: ${meeting.title} at ${meeting.dateTime}`);
...
// Migration function to fix existing meetings without dateTime
const migrateExistingMeetings = async () => {
    try {
        const meetingsWithoutDateTime = await Meeting.find({
            $or: [
        ...
        console.log(`🔄 Found ${meetingsWithoutDateTime.length} meetings to migrate`);

        for (const meeting of meetingsWithoutDateTime) {
            const dateTime = createDateTime(meeting.date, meeting.time);
            meeting.dateTime = dateTime;
            await meeting.save();
            console.log(`✅ Migrated meeting: ${meeting.title} -> ${dateTime}`);
        }
...
export const setupScheduler = (app, io) => {
    ...
    // Run migration for existing meetings
    migrateExistingMeetings();
