...
const getUserProfilePicture = async (email) => {
  try {
    ...
    const name = user ? `${user.firstName} ${user.lastName}` : email.split('@')[0];
    ...
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(email.split('@')[0])}&background=667eea&color=fff&size=64`;
  }
};
...
const getUserName = async (email) => {
  try {
    ...
    console.error(`Error getting name for ${email}:`, error);
...
// Integration function to create persistent sessions when users login
export const createPersistentSession = async (user, res, schedulerAPI) => {
  try {
    if (schedulerAPI && schedulerAPI.createUserSession) {
      const sessionToken = await schedulerAPI.createUserSession(
        user._id,
        user.email,
        user.firstName,
        user.lastName,
...
// Auth routes
export const setupAuthRoutes = (app, schedulerAPI = null) => {
...
app.get('/dashboard', authenticateUser, (req, res) => {
  ...
  app.get('/auth/google/callback',
    ...
    async (req, res) => {
      ...
      await createPersistentSession(req.user, res, schedulerAPI);
  ...
  app.post('/api/login/verify-code', async (req, res) => {
    try {
      ...
      // Create persistent session
      const sessionToken = await createPersistentSession(user, res, schedulerAPI);
...
export {
  ...
  getUserName, // Add this
  getUserProfilePicture, // Add this
